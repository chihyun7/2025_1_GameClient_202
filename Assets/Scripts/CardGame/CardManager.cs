...
public class CardManager : MonoBehaviour
{
    ...
    public GameObject cardPrefeb;               // 카드 프리펩
    ...
    {   
        ...
        if(Input.GetKeyDown(KeyCode.F))             // f키를 누르면 버린 카드르 덱이로 되돌리고 섞기
        {
            ReturnDiscardToDeck();
        }
    ...
    {
        List<CardData> tempDeck = new List<CardData>(deckCards);    // 임시 리스틍 카드 복사
        deckCards.Clear();
    ...
    public void ReturnDiscardToDeck()
    {
        ...
        deckCards.AddRange(discardCards);                                       // 버린 카드를 모두 데[ㄱ이추가
        discardCards.Clear();                                            //   버린 카드 더미 비우기
        ShuffleDeck();                                                          // 덱섞기

        Debug.Log("버린카드 " + deckCards.Count + " 장을 덱으로 되돌라고 섞었습니다.");
